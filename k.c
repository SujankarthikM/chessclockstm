...
void display(int digit) {
    ...
    // Clear all segment bits first (for segments connected to GPIOC, GPIOB, and GPIOA)
    GPIOC->ODR &= ~((1U << 14) );    // Clear segments a and b
    GPIOB->ODR &= ~((1U << 12) | (1U << 13) | (1U << 14) | (1U << 15)); // Clear segments c, d, e, f
    GPIOA->ODR &= ~((1U << 8 )|(1U << 5));                    // Clear segment g

    // Set each segment based on the bit values in segments
    if (segments & 0b00000001) GPIOC->ODR |= (1U << 14); // Segment a
    if (segments & 0b00000010) GPIOA->ODR |= (1U << 5); // Segment b
    if (segments & 0b00000100) GPIOB->ODR |= (1U << 12); // Segment c
    if (segments & 0b00001000) GPIOB->ODR |= (1U << 13); // Segment d
    if (segments & 0b00010000) GPIOB->ODR |= (1U << 14); // Segment e
    if (segments & 0b00100000) GPIOB->ODR |= (1U << 15); // Segment f
    if (segments & 0b01000000) GPIOA->ODR |= (1U << 8);  // Segment g
}
...
int main(void) {
    ...
    GPIOC->MODER &= ~(0x3 << 28); // Clear mode bits for PC14
		...
		GPIOA->MODER |= (0x1 << 10);  // Set PA5 to output mode (01)
